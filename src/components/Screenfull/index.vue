<template>
  <div class="center-row" style="color:#4e7afb;">
    <span style="padding: 0 12px;font-size:18px;">{{ !isQuit?'全屏':'退出' }}</span>
    <svg
      v-if="!isQuit"
      t="1508738709248"
...
export default {
  ...
  mounted() {
    window.onresize = () => {
      this.$store.commit('TOGGLE_SCREENFULL', screenfull.isFullscreen)
    }
  },
  methods: {
    handleScreenfull() {
